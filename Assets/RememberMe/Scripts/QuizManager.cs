...
//    private Image questionImage; //Buat gambar soal
    ...
    private void setQuestion()
    {
        ...
        if(currentQuestionIndex  == 6 && currentCorrectAnswer != 6)
        {
    ...
    public void SelectedOption(WordData WordData)
    {
        ...
        for(int i = 0; i < answerWord.Length; i ++)
        {
            if (currentAnswerIndex == clueIndex[countIndex2])
            {
                currentAnswerIndex++;
                countIndex2++;
            }
    ...
    public void ClueBtnText()
    {
        if(currentClue > 0)
        {
            ...
            for(int i = 0; i < answerWord.Length; i++)
            {
               // Debug.Log(answerWordArray[currentAnswerIndex]);
                if(char.ToUpper(answerWord[i]) == char.ToUpper(answerWord[currentAnswerIndex])) // mencari apakah ada huruf yg sama dgn index saat ini
                {
                    ...
                    countIndex += 1;
                    //Debug.Log(clueIndex[countIndex]);
